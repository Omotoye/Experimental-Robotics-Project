...
class KnowledgeManager:
	...
	def knowledge_clbk(self, msg):
		...
		if msg.goal == "update":
			...
			self.call_reasoner()
			self.add_objectprop_to_ind(self.objectprop_name, self.hypo_name, self.ind_name)
			self.call_class_disjoint(self.class_name)
			self.call_reasoner()
	...
	def get_hypo_data(self, hypo_id):
		hypo_name, = list(hypo[int(i)])
		
		for items in hypo[int(i)][hypo_name]:
			key, = list(items) 
			value = items[key]
			if key == 'who':
				who = value
	...
	def add_objectprop_to_ind(self, objectprop_name, ind_name, value_obj_name):
		...
		"""
		try:
			res = self.call('ADD', 'OBJECTPROP', 'IND', [objectprop_name, ind_name, value_obj_name])
